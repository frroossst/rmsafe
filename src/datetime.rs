...
struct Tm { year: i32, month: u8, day: u8, hour: u8, min: u8, sec: u8 }

fn unix_to_utc(mut secs: u64) -> Tm {
    let sec = (secs % 60) as u8; secs /= 60;
    let min = (secs % 60) as u8; secs /= 60;
    let hour = (secs % 24) as u8; secs /= 24;
    let mut year = 1970;
    ...
    Tm { year, month, day, hour, min, sec }
}
